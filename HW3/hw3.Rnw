...
<<start,message=FALSE>>=
houseprices <- read.table("http://www.stat.columbia.edu/~madigan/W2025/data/ToyExample.dat", header=TRUE)
attach(houseprices)
@
Then I plot the distribution of each predictor for responses y=0 and y=1.  This provides some intuition about how the features might help us differentiate between records that are classfied by 0 and 1.  In the graphs below, we notice two things: 1) x2's distribution for records in which y is 1 seems to be centered noticably higher than for records in which y is 0 and 2) that while both x1 and x2 have fairly symmetrical densities, x3 is skewed right, indicating we might have trouble fitting a least squares regression model because of the normality assumption
<<loadinglibs,message=FALSE, echo=FALSE>>=
library(BMA)
library(ggplot2)
library(popbio)
@
<<exploratorydataanalysis,message=FALSE, echo=FALSE>>=
...
@
We can confirm that there are indeed issues fitting a logistic regression model to x3.  To get rid of the problem with the pattern of the residuals, we transform x3 and instead fit the model to log(x3):
<<logx3 >>=
...
@
Having established that each of the predictors indivdually are acceptable to use in logistic regression, I then proceed to model selection by searching the model space of the combinations of x1, x2, and log(x3).  To start this procedure, I define functions to enumerate the various models of 

<<apply_formula,tidy=FALSE>>=

apply_formula <- function(model, cv=5){ #make the default value for this function 5
    cv.model <- CVlm(houseprices,glm(model, family=binomial, data=houseprices),m=cv)
    cvmse <-mean((cv.model$sale.price - cv.model$cvpred)^2) 
}

FORMULAS <- all_possible_models(
    x.column.names=colnames(houseprices[2:4]),
    y.col.name=colnames(houseprices[1])
    )
...
Once I have all possible combinations of predictors, I fit a logistic regression model to each of them and store the resulting AIC, BIC, and cross validation error.
